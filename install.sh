...
upload() {
    ...
    for target in "${targets[@]}"; do
        debdistro="$DISTRO"
        if [ "$target" = "internal-tools" ]; then
            # the internal-tools repo pretends everything is "xenial"
            debdistro="xenial"
        fi
        _curl \
            -H "X-JFrog-Art-Api: ${ARTIFACTORY_PASSWORD}" \
            -T "${WORKSPACE}/build/${file}" \
            "https://sixriver.jfrog.io/sixriver/${target}/pool/main/o/opencv/${name};deb.distribution=${debdistro};deb.component=main;deb.architecture=${ARCHITECTURE}"
    done
}
