...
// Disable some warnings which are caused with MUSA headers
#if defined(_MSC_VER)
...
int main()
{
    std::cout << "MUSA support is required (OpenCV CMake parameter 'WITH_MUSA' must be true)." << std::endl;
    return 0;
}
...
int main()
{
    int num_devices = getMusaEnabledDeviceCount();
    if (num_devices < 2)
    {
    ...
    {
        cv::musa::printShortMusaDeviceInfo(i);

        DeviceInfo dev_info(i);
        if (!dev_info.isCompatible())
        {
            std::cout << "MUSA module isn't built for GPU #" << i << " ("
