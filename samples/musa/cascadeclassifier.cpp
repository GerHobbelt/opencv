...
#include "opencv2/musaobjdetect.hpp"
...
static void convertAndResize(const GpuMat& src, GpuMat& gray, GpuMat& resized, double scale)
{
    if (src.channels() == 3)
    {
        cv::musa::cvtColor( src, gray, COLOR_BGR2GRAY );
    }
    else
    {
        gray = src;
    ...
    if (scale != 1)
    {
        cv::musa::resize(gray, resized, sz);
    }
    else
    {
        resized = gray;
...
int main(int argc, const char *argv[])
{
    ...
    if (getMusaEnabledDeviceCount() == 0)
    {
        return cerr << "No GPU found or the library is compiled without MUSA support" << endl, -1;
    }
    ...
    Ptr<musa::CascadeClassifier> cascade_gpu = musa::CascadeClassifier::create(cascadeName);
