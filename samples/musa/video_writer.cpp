...
#if defined(HAVE_OPENCV_MUSACODEC) && defined(_WIN32)
...
int main(int argc, const char* argv[])
{
    ...
    for (int i = 1;; ++i)
    {
        ...
        if (d_writer.empty())
        {
            std::cout << "Open MUSA Writer" << std::endl;

            const cv::String outputFilename = "output_gpu.avi";
            d_writer = cv::musacodec::createVideoWriter(outputFilename, frame.size(), FPS);
        }
...
int main()
{
    std::cout << "OpenCV was built without MUSA Video encoding support\n" << std::endl;
    return 0;
}
