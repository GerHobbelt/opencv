...
static Ptr<cv::superres::DenseOpticalFlowExt> createOptFlow(const string& name, bool useGpu)
{
    ...
    {
        if (useGpu)
            return cv::superres::createOptFlow_Farneback_MUSA();
        else
    ...
    {
        if (useGpu)
            return cv::superres::createOptFlow_DualTVL1_MUSA();
        else
            return cv::superres::createOptFlow_DualTVL1();
    }
    else if (name == "brox")
        return cv::superres::createOptFlow_Brox_MUSA();
    else if (name == "pyrlk")
        return cv::superres::createOptFlow_PyrLK_MUSA();
    else
...
int main(int argc, const char* argv[])
{
    CommandLineParser cmd(argc, argv,
        ...
        "{ g gpu        | false     | CPU as default device, musa for MUSA }"
    ...
    bool useMusa = gpuOption.compare("musa") == 0;
    Ptr<SuperResolution> superRes;

    if (useMusa)
        superRes = createSuperResolution_BTVL1_MUSA();
    else
        superRes = createSuperResolution_BTVL1();

    Ptr<cv::superres::DenseOpticalFlowExt> of = createOptFlow(optFlow, useMusa);
    ...
    if (useMusa)
    {
        ...
        try
        {
            frameSource = createFrameSource_Video_MUSA(inputVideoName);
            Mat frame;
    ...
    // skip first frame, it is usually corrupted
    {
        ...
        cout << "Mode            : " << (useMusa ? "MUSA" : "CPU") << endl;
    }
