...
App::App(int argc, const char* argv[])
{
    ...
    if (src.channels() == 3)
    {
        ...
        musa::GpuMat src4ch;
        musa::cvtColor(src, src4ch, COLOR_BGR2BGRA);
        src = src4ch;
    }

    help();

    musa::printShortMusaDeviceInfo(musa::getDevice());
}
...
void App::OpenClose()
{
    ...
    if (n < 0)
    {
        Ptr<musa::Filter> openFilter = musa::createMorphologyFilter(MORPH_OPEN, src.type(), element);
        openFilter->apply(src, dst);
    }
    else
    {
        Ptr<musa::Filter> closeFilter = musa::createMorphologyFilter(MORPH_CLOSE, src.type(), element);
...
void App::ErodeDilate()
{
    ...
    if (n < 0)
    {
        Ptr<musa::Filter> erodeFilter = musa::createMorphologyFilter(MORPH_ERODE, src.type(), element);
        erodeFilter->apply(src, dst);
    }
    else
    {
        Ptr<musa::Filter> dilateFilter = musa::createMorphologyFilter(MORPH_DILATE, src.type(), element);
