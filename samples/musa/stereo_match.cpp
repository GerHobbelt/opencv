...
#include "opencv2/musastereo.hpp"
...
private:
    ...
    Mat left, right;
    musa::GpuMat d_left, d_right;

    Ptr<musa::StereoBM> bm;
    Ptr<musa::StereoBeliefPropagation> bp;
    Ptr<musa::StereoConstantSpaceBP> csbp;
...
{
    cv::musa::printShortMusaDeviceInfo(cv::musa::getDevice());
...
void App::run()
{
    ...
    bm = musa::createStereoBM(p.ndisp);
    bp = musa::createStereoBeliefPropagation(p.ndisp);
    csbp = cv::musa::createStereoConstantSpaceBP(p.ndisp);

    // Prepare disparity map of specified type
    Mat disp(left.size(), CV_8U);
    musa::GpuMat d_disp(left.size(), CV_8U);
