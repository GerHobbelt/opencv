...
private:
    ...
    GpuMat d_rightFrame;
    GpuMat d_disparity;
    Ptr<musa::StereoBM> d_alg;
};

StereoSingleGpu::StereoSingleGpu(int deviceId) : deviceId_(deviceId)
{
    musa::setDevice(deviceId_);
    d_alg = musa::createStereoBM(256);
}
...
{
    musa::setDevice(0);
    d_algs[0] = musa::createStereoBM(256);

    musa::setDevice(1);
    d_algs[1] = musa::createStereoBM(256);
}
...
int main(int argc, char** argv)
{
    ...
    const int numDevices = getMusaEnabledDeviceCount();
    if (numDevices != 2)
    {
    ...
    for (int i = 0; i < numDevices; ++i)
    {
        ...
        {
            cerr << "MUSA module wasn't built for GPU #" << i << " ("
                 << devInfo.name() << ", CC " << devInfo.majorVersion()
                 << devInfo.minorVersion() << endl;
            return -1;
        }

        printShortMusaDeviceInfo(i);
    }
